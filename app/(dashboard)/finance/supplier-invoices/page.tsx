...
export default function SupplierInvoicesPage() {
  ...
  const { data, isLoading, isError, error, refetch } = useSupplierInvoiceLedger(supplierId ?? "", {
    page,
    pageSize: PAGE_SIZE,
  });
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {!supplierId ? (
        <EmptyState title="Select a supplier" description="Choose a supplier above to view their invoices." />
      ) : isLoading ? (
        ...
        <div className="flex flex-col gap-4">
          ...
          {data.invoices.length === 0 ? (
            <EmptyState title="No invoices" description={`${supplierLabel} has no invoices yet.`} />
          ) : (
            <>
