...
import { useSupplierInvoiceLedger, usePaySupplierInvoice } from "@/features/finance/hooks/use-finance";
...
export default function SupplierInvoiceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, isLoading, isError, error, refetch } = useSupplierInvoiceLedger(supplierId, { pageSize: 100 });
  const payInvoice = usePaySupplierInvoice(supplierId);
    ...
    return (
      <div className="flex flex-col gap-6">
        ...
        <EmptyState
          ...
          description="Open this invoice from the supplier invoices list — there is no standalone invoice lookup endpoint."
        />
      </div>
    );
  }
  ...
  if (!invoice) {
    return <EmptyState title="Invoice not found" description="This invoice could not be located for the selected supplier." />;
  }
      ...
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            <PdfDownloadButton path={`/Invoice/download/${invoice.id}`} fallbackFilename={`invoice-${invoice.id}.pdf`} />
            {outstanding > 0 && (
              <button
                type="button"
      ...
      <dl className="grid max-w-md grid-cols-2 gap-4 rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
        ...
        {invoice.stockId && (
          <div>
            ...
            <dd className="mt-0.5 text-sm text-zinc-900 dark:text-zinc-100">{invoice.stockId}</dd>
          </div>
        )}
      </dl>
