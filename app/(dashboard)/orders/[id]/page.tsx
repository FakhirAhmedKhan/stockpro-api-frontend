...
export default function OrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <dl className="grid max-w-lg grid-cols-2 gap-4 rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
        <div>
          ...
          <dd className="mt-0.5 text-sm text-zinc-900 dark:text-zinc-100">
            <Link href={ROUTES.customerDetail(order.customerId)} className="hover:underline">
              {order.customerId}
            </Link>
          </dd>
        </div>
        <div>
          <dt className="text-xs font-medium uppercase tracking-wide text-zinc-500">Stock</dt>
          <dd className="mt-0.5 text-sm text-zinc-900 dark:text-zinc-100">{order.stockId}</dd>
        </div>
        <div>
          <dt className="text-xs font-medium uppercase tracking-wide text-zinc-500">Quantity</dt>
          <dd className="mt-0.5 text-sm text-zinc-900 dark:text-zinc-100">{order.quantity}</dd>
        </div>
        <div>
        ...
        <div>
          ...
          <dd className="mt-0.5 text-sm text-zinc-900 dark:text-zinc-100">
            {formatCurrency(Number(order.totalPrice) - Number(order.paidAmount))}
          </dd>
        </div>
        <div>
          <dt className="text-xs font-medium uppercase tracking-wide text-zinc-500">Invoice</dt>
          <dd className="mt-0.5 text-sm text-zinc-900 dark:text-zinc-100">{order.invoiceId}</dd>
        </div>
      </dl>

      <div>
        <h2 className="mb-2 text-sm font-medium text-zinc-700 dark:text-zinc-300">Products</h2>
