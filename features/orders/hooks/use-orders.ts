...
export function useOrdersCache() {
  ...
  const list =
    queryClient.getQueryData<OrderResponseDto[]>(queryKeys.orders()) ?? [];
  return list;
}
  ...
  return useMutation({
    ...
    onSuccess: (order) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["customer-ledger", order.customerId],
      });
    ...
    onError: (error: ApiError) => {
      if (error.status === 409) {
        toast.error(
          "One or more products are no longer available. Please review and try again.",
        );
      } else {
...
export function useUpdateOrder(orderId: string) {
  ...
  return useMutation({
    mutationFn: (payload: UpdateOrderPayload) =>
      orderApi.update(orderId, payload),
    onSuccess: (order) => {
      appendToOrderListCache(queryClient, order);
      queryClient.invalidateQueries({ queryKey: ["products"] });
      queryClient.invalidateQueries({ queryKey: ["customer-invoices"] });
    ...
    onError: (error: ApiError) => {
      if (error.status === 409) {
        toast.error(
          "Inventory changed since you loaded this order. Please review and try again.",
        );
      } else {
